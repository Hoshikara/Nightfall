...
function Console.new(ctx, window)
  ---@type Console
  local self = {
    base = Image.new({ path = "gameplay/console/base.png" }),
    buttons = {},
    ctx = ctx,
    knobs = {},
    window = window,
  ...
  for i = 1, 6 do
    self.buttons[i] = {
      overlay = Image.new({ path = ("gameplay/console/button_%d.png"):format(i) }),
      timer = 0,
    }
  end
  ...
  for i = 1, 2 do
    self.knobs[i] = {
      glow = Image.new({ path = ("gameplay/console/glow_%d.png"):format(i) }),
      glowTimer = 0,
      ring = Image.new({ path = ("gameplay/console/ring_%d.png"):format(i) }),
      ringTimer = 0,
    }
  end
