...
---@param settings GameplaySetting[]
---@return number
...
local function handleOptionsSetting(setting, params)
	...
	setting.valueLabel = makeLabel("Medium", setting.text, 30)
...
local function makeSetting(params)
	...
	if params.name then
		setting.name = makeLabel("Medium", params.name, 30)
	end
...
local function makeTab(heading, tabName, statusKey)
	...
	tab.height = getHeight(tab.settings)
...
local GameplaySettingsTabs = {
	Chain = makeTab("CHAIN", "Chain", "showChain"),
	Earlate = makeTab("EARLY / LATE", "Earlate", "showEarlate"),
