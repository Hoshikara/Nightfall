...
local legendTable = {
	...
	{ ["labelSingleLine"] = gfx.CreateLabel("PLAY", 16, 0), ["labelMultiLine"] = gfx.CreateLabel("PLAY", 16, 0),
...
check_or_create_cache = function(song, full)
	...
	songCache[song.id]["percent_value"] ~= song.bestScore) then
		songCache[song.id]["percent"] = gfx.CreateLabel(string.format("%u%% Complete",
			math.max(0, (song.bestScore - 8000000) // 10000)), 35, 0)
...
draw_selected = function(song, x, y, w, h)
	...
	for i, chart in ipairs(song.charts) do
		...
		gfx.ImageRect(xoff + imageXPos, img_row * imageSize + y + yMargin + yPadding, imageSize, imageSize,
	...
	local boxh, boxw = 0, nil
	local starty, startx = 0, nil
